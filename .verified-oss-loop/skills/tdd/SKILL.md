...
The unit command is detected at onboard time (`python scripts/validate.py`). If that placeholder is still `unknown`, detect the tree (`pyproject.toml`, `package.json`, `Cargo.toml`, `go.mod`) and pin the command in the PR. Do not copy another project's test runner.
