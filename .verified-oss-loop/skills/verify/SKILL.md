...
1. **Unit** — `python scripts/validate.py` on the touched surface.
2. **TDD** — red command, then green command (`skills/tdd/SKILL.md`).
3. **Mutation** — `n/a` if that is not `n/a`. A surviving mutant is a missing assertion. If the stack has no mutator, write `n/a`. Do not invent a score.
4. **Runtime** — `python scripts/build_site.py`. If the project has no runtime check, write `n/a` and say what you did not run.
